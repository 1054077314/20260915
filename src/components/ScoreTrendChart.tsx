...
export const ScoreTrendChart: React.FC<ScoreTrendChartProps> = ({
  ...
  // Chart Mode:
  // "REAL_ONLY": 严格只显示真实参测的期数数据点与轨迹（未参测为 null，不连线/虚构，真实还原客观历史）
  // "FULL_EXTRAPOLATED": 基于各模型综合攻坚上限与测试题难度的回测映射走势（保留全景参考，并明确标记为推导）
  ...
  React.useEffect(() => {
    if (selectedModel?.id && REAL_MODEL_HISTORIES[selectedModel.id]) {
  ...
  const highlightModel = currentHighlightId ? REAL_MODEL_HISTORIES[currentHighlightId] : null;

  // Custom Tooltip showing real test verification status at hovered episode
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload;

      // Collect scores of visible models
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between border-b border-white/[0.08] pb-4 mb-6 gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {/* Clear Disclaimer Tag responding directly to user query */}
            <span className="inline-flex items-center gap-1 font-mono-code text-[11px] px-2 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/30">
        ...
        <div className="flex items-center gap-3 font-mono-code text-xs flex-wrap">
          {/* REAL_ONLY vs FULL_EXTRAPOLATED Mode Switcher */}
          <div className="flex items-center bg-zinc-900 border border-white/[0.12] p-0.5 rounded-lg shadow-inner">
            <button
              ...
              title="仅在模型实际出镜、有真实评测视频的期数绘制数据点（断点真实呈现）"
            >
            ...
            <button
              ...
              title="根据各模型实战斩杀线水平（黄金/钻石/王者），推导其在 12 期难度下的能力映射曲线"
            >
      ...
      {/* Explanation Banner: Explaining Why and How Curves are Drawn */}
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-3 mb-6 scrollbar-none">
        ...
        {Object.values(REAL_MODEL_HISTORIES).map((m) => {
              ...
              {visibleModels.map((model) => {
                ...
                return (
                  <Line
                    ...
                    // connectNulls=false ensures we DO NOT draw fake lines across missing episodes in REAL_ONLY mode
                    connectNulls={false}
                    dot={
                      isHighlighted
                        ? {
